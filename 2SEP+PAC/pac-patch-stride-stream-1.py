...
import copy
import shiying1 as compress_model
## 原始方法，未加同步操作
...
# Dataset parameters
flags.DEFINE_integer('seq_len',48, 'Maximum sequence length (L).')
flags.DEFINE_integer('vocab_size', 256, 'Vocabulary size of data.')
flags.DEFINE_string('input_dir', '../../../data/dickens', 'input data dir')
flags.DEFINE_string('prefix', '_dickens', 'output dir')
...
def encode(temp_dir, compressed_file, FLAGS, series, train_data, last_train_data,queue,enc):
  ...
  # 需要修改，能被4整除
  record_num = 1
  for train_index in range(0,iter_num,4):
    ...
    for i in range(0,stream_num):
      with torch.cuda.stream(streamPooling[i]):
        ...
        # GPU COMPUTING
        train_loss, logits = model.full_loss(train_batch_list[i],with_grad=True,inputs_lista=train_batch_list,streamPoolingb=streamPooling)
        #GPU
        optimizer.step()
        #CPU
        optimizer.zero_grad(set_to_none=True)
