...
class PatchTST_backbone(nn.Module):
    def __init__(self,
        ...
        # padding_patch 参数在这个上下文中是用来确定是否以及如何对输入序列进行填充的。
        # 在某些情况下，当输入序列的长度不能被给定的 patch 长度或者 stride 整除时，我们可能需要对输入序列进行填充。
        self.padding_patch = padding_patch
        patch_num = int((seq_len - patch_len) / stride + 1)
        if padding_patch == 'end':  # can be modified to general case

            # nn.ReplicationPad1d((0, stride)) 是 PyTorch 中的一个函数，它用于在一维输入的两侧添加填充。
            # 在这个函数中，参数 (0, stride) 表示在序列的起始处添加 0 个填充，而在序列的末尾添加 stride 个填充。
            self.padding_patch_layer = nn.ReplicationPad1d((0, stride))
            patch_num += 1

        # Backbone
        self.backbone = TSTiEncoder(seq_len, patch_num=patch_num, patch_len=patch_len, max_seq_len=max_seq_len,
        ...
        # Head
        self.head_nf = patch_len * patch_num
        ...
        self.input_map = torch.nn.Embedding(256, vocab_dim // 2)
        self.output_logit_map = torch.nn.Linear(256, 256)
    def forward(self, x):  # z: [bs  x seq_len x nvars]
        x = self.input_map(x)
        x = self._concat_pos_embs(x, 0)
        bs, seqlen, vlen = x.shape
    
        x = x.reshape(bs, seqlen // 4, vlen*4)
        z = self.backbone(x)  # z: [bs x nvars x d_model x patch_num]
        z = self.output_logit_map(z)
        return z
...
class TSTiEncoder(nn.Module):  # i means channel-independent
    def __init__(self, c_in, patch_num, patch_len, max_seq_len=1024,
        ...
        # Input encoding
        q_len = patch_num
        self.W_P = nn.Linear(self.patch_len, d_model)  # Eq 1: projection of feature vectors onto a d-dim vector space
        self.seq_len = q_len
    ...
    def forward(self, x) -> Tensor:  # x: [bs x nvars x patch_len x patch_num]
        ...
        # """
        # 在这段代码中，self.W_P 是一个线性变换（全连接层），它作用在 patch_len 这个维度上，而保持 bs, nvars, patch_num 这三个维度不变。
        # 因此，这个线性变换在每一个通道 (nvars 维度) 上都是独立地应用的，实现了对每个通道独立处理的效果。
        # """
        # x = x.permute(0,1,3,2)         # x: [bs x nvars x patch_num x patch_len]
        # print(x.shape)
        # u = torch.reshape(x, (x.shape[0]*x.shape[1],x.shape[2],x.shape[3]))      # u: [bs * nvars x patch_num x d_model]
        # print(u.shape)                           # u: [bs * nvars x patch_num x d_model]
